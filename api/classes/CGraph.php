    ...
    **/
    ...
    class CGraph extends CGraphGeneral {
        ...
        protected function inherit($graph, $hostids = null) {
            ...
            foreach($chdHosts as $chdHost) {
                ...
                if ($chdGraph = reset($chdGraphs)) {
                    if (zbx_strtolower($tmpGraph['name']) != zbx_strtolower($chdGraph['name']) && $this->exists(array('name'    => $tmpGraph['name'],
                                                                                                                      'hostids' => $chdHost['hostid']
                            ))
                    ) {
        ...
        protected function applyQueryNodeOptions($tableName, $tableAlias, array $options, array $sqlParts) {
            ...
            if ($options['graphids'] === null && $options['templateids'] === null && $options['hostids'] === null && $options['groupids'] === null && $options['itemids'] === null) {
            ...
            foreach($graphs as $graph) {
                ...
                foreach($graph['gitems'] as $graphItem) {
                    ...
                    if (!in_array($item['value_type'], $allowedValueTypes)) {
                        self::exception(ZBX_API_ERROR_PARAMETERS, _s('Cannot add a non-numeric item "%1$s" to graph "%2$s".', $item['name'], $graph['name']));
                    }
                }

                // Y axis min
                if (isset($graph['ymin_itemid']) && $graph['ymin_itemid'] && isset($graph['ymin_type']) && $graph['ymin_type'] == GRAPH_YAXIS_TYPE_ITEM_VALUE) {
                ...
                // Y axis max
                if (isset($graph['ymax_itemid']) && $graph['ymax_itemid'] && isset($graph['ymax_type']) && $graph['ymax_type'] == GRAPH_YAXIS_TYPE_ITEM_VALUE) {
