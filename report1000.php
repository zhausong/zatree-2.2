<?php
    ...
    **/
    ...
    $page['title'] = _('New report');
    $page['file'] = 'report1000.php';
    ...
    if ($availabilityReportMode == AVAILABILITY_REPORT_BY_TEMPLATE) {
        if (get_request('hostgroupid') && !API::HostGroup()->isReadable(array($_REQUEST['hostgroupid'])) || get_request('filter_groupid') && !API::HostGroup()->isReadable(array($_REQUEST['filter_groupid'])) || get_request('filter_hostid') && !API::Host()->isReadable(array($_REQUEST['filter_hostid']))) {
            access_deny();
        }
        ...
        if (get_request('filter_groupid') && !API::HostGroup()->isReadable(array($_REQUEST['filter_groupid'])) || get_request('filter_hostid') && !API::Host()->isReadable(array($_REQUEST['filter_hostid']))) {
            access_deny();
        }
    }
     ...
     * Header
     */
    $triggerData = isset($_REQUEST['triggerid']) ? API::Trigger()->get(array(
    ...
    if ($triggerData) {
        ...
        if ($availabilityReportMode == AVAILABILITY_REPORT_BY_TEMPLATE) {
            ...
            foreach($groups as $group) {
                $groupsComboBox->addItem($group['groupid'], get_node_name_by_elid($group['groupid'], null, NAME_DELIMITER) . $group['name']);
            }
            ...
            foreach($hostGroups as $hostGroup) {
                $hostGroupsComboBox->addItem($hostGroup['groupid'], get_node_name_by_elid($hostGroup['groupid'], null, NAME_DELIMITER) . $hostGroup['name']);
            }
        ...
        elseif ($availabilityReportMode == AVAILABILITY_REPORT_BY_HOST) {
            ...
            foreach($hosts as $host) {
                $hostsComboBox->addItem($host['hostid'], get_node_name_by_elid($host['hostid'], null, NAME_DELIMITER) . $host['name']);
            }
        ...
        $filterForm->addItemToBottomRow(new CButton('filter_rst', _('Reset'), 'javascript: var url = new Curl(location.href); url.setArgument("filter_rst", 1); location.href = url.getUrl();'));
        ...
        foreach($triggers as $trigger) {
            ...
            $triggerTable->addRow(array(
                ...
                ($_REQUEST['filter_hostid'] == 0 || $availabilityReportMode == AVAILABILITY_REPORT_BY_TEMPLATE) ? $trigger['hosts'][0]['name'] : null,
                new CLink($trigger['description'], 'events.php?triggerid=' . $trigger['triggerid'] . '&source=' . EVENT_SOURCE_TRIGGERS),
                new CSpan(sprintf('%.4f%%', $availability['true']), 'on'),
                new CSpan(sprintf('%.4f%%', $availability['false']), 'off'),
                new CLink(_('Show'), 'report2.php?filter_groupid=' . $_REQUEST['filter_groupid'] . '&filter_hostid=' . $_REQUEST['filter_hostid'] . '&triggerid=' . $trigger['triggerid'])
            ));
        }
