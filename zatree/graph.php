...
<head>
    ...
    <?php
        $changeType = (isset($_REQUEST["changeType"]) && $_REQUEST["changeType"]) ? $_REQUEST["changeType"] : '1hour';
    ?>
...
<?php
    ...
    /* 条件不从cookie里面获取 */
    if (!isset($_GET["itemkey"]) && !isset($_GET["stime"]) && !isset($_GET["endtime"]) && !isset($_GET["order_key"]) && !isset($_GET["order_type"])) {
        if (isset($_COOKIE['stime'])) {
            ...
            $stime = (isset($_GET["stime"]) && $_GET["stime"] != '') ? $_GET["stime"] : date("Y-m-d H:i:s", $curtime - 3600);
    ...
    if (isset($_REQUEST["host"]) && $_REQUEST["host"]) {
        $host_info_by = API::Host()->get(array(
            "output" => "extend",
            "filter" => array('host' => $_REQUEST["host"])
        ));
...
<form method="get" style="font-size:8px;text-align:left;padding-left:10px;" id="searchForm">
    <div>
        ...
        开始时间：<input type="text" value="<?php echo $stime; ?>" width="100px;" class="Wdate"
                    onfocus="WdatePicker({dateFmt: 'yyyy-MM-dd HH:mm:ss'})" id="stime" name="stime"/> &nbsp;
        结束时间：<input type="text" id="endtime" value="<?php echo $endtime; ?>" width="100px;" class="Wdate"
                    onfocus="WdatePicker({dateFmt: 'yyyy-MM-dd HH:mm:ss'})" name="endtime"/>&nbsp;
        ...
        <input type="button" onclick="javascript:window.parent.location.href = '<?php echo $url_http; ?>'"
               value="回到首頁"/>
    </div>
    <div style=" margin: 10px 0px 0px 5px;font-size: 14px; " id="quickdiv"><a href="javascript:void(0);"
                                                                              onclick="changeTime('1hour');">1小时</a>&nbsp;&nbsp;<a
            href="javascript:void(0);" onclick="changeTime('2hour');">2小时</a>&nbsp;&nbsp;<a href="javascript:void(0);"
                                                                                            onclick="changeTime('1days');">1天</a>&nbsp;&nbsp;<a
            href="javascript:void(0);" onclick="changeTime('2days');">2天</a>&nbsp;&nbsp;<a href="javascript:void(0);"
                                                                                           onclick="changeTime('7days');">7天</a>&nbsp;&nbsp;<a
            href="javascript:void(0);" onclick="changeTime('30days');">30天</a>&nbsp;&nbsp;<a href="javascript:void(0);"
                                                                                             onclick="changeTime('1years');">1年</a>&nbsp;&nbsp;<a
            href="javascript:void(0);" onclick="changeTime('2years');">2年</a></div>
</form>

<p></p>
<?php
    $order_list_result = $order_list_result_page = array(); //记录结果信息数
    $graphids = null;
    if ((isset($_REQUEST["hostid"]) && $_REQUEST["hostid"] > 0) || ($group_class != '' && $group_class > 0) || $hostid) {
        ...
        if ($group_class == '') {
            ...
            if (!is_null($graphids)) {
                $items_list = API::Graph()->get(array("graphids" => $graphids, "output" => "extend"));
            }
        } else {
            //查询分组里面的所有机器
            $host_ids = array();
        ...
        $list = array(
            'list_item' => $items_list,
            'parame'    => array(
                'stime'            => strtotime($stime),
                'period'           => $period,
                'sizeX'            => $width,
                'item_name_search' => $itemkey
            )
        );
        ...
        if (count($order_list_result) > 0) {
            ...
            if (empty($itemkey)) {
                $page = new page($order_list_result, array(
                    'total'     => count($order_list_result),
                    'url'       => $url,
                    'nowindex'  => $page,
                    'searchkey' => $itemkey
                ));
                ...
                $page = new page(array_values($search_list), array(
            ...
            if ($group_class != '' && $group_class > 0) {
                echo "<div>当前主机组没有graph数据</div>";
            } else {
                echo "<div>当前主机没有graph数据</div>";
            }
        }
        if (isset($page_link)) {
            ?>
        ...
        foreach($order_list_result_page as $result) {
            ...
            <div style="width:357px;float: left; text-align: left;margin: 5px 3px 0px 0px;">
                ...
                <a class="fancybox" href="<?php echo $big_graph_change; ?>">
                    <img src="<?php echo $small_graph; ?>" width="357" height="211"
                         style="float:left;padding-top:4px;padding-left:4px;"/> </a>
            </div>
